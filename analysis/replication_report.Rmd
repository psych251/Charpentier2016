...
accuracy <- accuracy %>%
        ...
        group_by(model) %>%
        summarise(mean.accuracy = mean(accuracy), ci = ci(accuracy))

ggplot() +  
        stat_summary(data=accuracy[1,], aes(x = model, y = mean.accuracy), fill = "orange", fun.y="identity", geom="bar", width=.5) +
        stat_summary(data=accuracy[2,], aes(x = model, y = mean.accuracy), fill = "darkblue", fun.y="identity", geom="bar", width=.5) +
        stat_summary(data=accuracy[3:7,], aes(x = model, y = mean.accuracy), fun.y="identity", geom="bar", width=.5) +
        ylim(c(0, 1)) +
