...
gamble_expect_model.fit <- list()
gamble_experience_model.fit <- list()
gamble_value_model.fit <- list()
gamble_log_value_model.fit <- list()
...
for(i in 1:nparticipant){
        gamble_expect_model.fit <- c(gamble_expect_model.fit, gamble_log_fit(gamble_block, feel_expect_mod3.fit[[i]]))
        gamble_experience_model.fit <- c(gamble_experience_model.fit, gamble_log_fit(gamble_block, feel_expect_mod3.fit[[i]]))
        gamble_value_model.fit <- c(gamble_value_model.fit, glm(data = gamble_block %>% filter(id == i), gamble ~ 0 + win + lose + sure, family = binomial))
        gamble_log_value_model.fit <- c(gamble_log_value_model.fit, glm(data = gamble_block %>% filter(id == i), gamble ~ 0 + log(win) + log(lose) + log(sure), family = binomial))
}
